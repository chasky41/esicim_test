...
<head>
    ...
    <style>
        ...
        /* --- MODIFICATION HEADER --- */
        .sio-hero {
            background-image: url('photos/header1.png');
    ...
    gap: 25px; 
    margin-top: 430px;
    margin-right: 50px; /* ✅ AJOUTEZ CETTE LIGNE */
}
        ...
        /* La classe principale pour le layout en deux colonnes */
.two-column-section {
    display: flex;            /* Active le mode Flexbox pour aligner les enfants horizontalement */
    gap: 40px;                /* Crée un espace de 40px entre l'image et le texte */
    align-items: center;      /* Centre verticalement les deux blocs l'un par rapport à l'autre */
}

/* Définit comment la colonne de texte doit se comporter */
.two-column-section .text-content {
    flex: 1; /* Permet au bloc de texte de grandir pour occuper l'espace disponible */
}

/* Définit comment la colonne de l'image doit se comporter */
.two-column-section .image-content {
    flex: 1;            /* Donne une flexibilité égale au bloc de l'image */
    max-width: 500px;   /* Mais limite sa largeur maximale pour ne pas qu'elle soit trop grande */
}

/* Assure que l'image remplit son conteneur */
.two-column-section img {
    width: 100%;
    border-radius: 10px;
}

/* Règle pour les écrans plus petits (tablettes, mobiles) */
@media (max-width: 992px) {
    .two-column-section {
        flex-direction: column; /* Remet les éléments l'un sur l'autre (verticalement) */
    }
}
